...
class Rule:
    ...
    Parameters:
    left : array-like of size (n_antecedents,3) 
                            - indices of the features used
    ...
    ovr_class : int - the class the ovr model is built for
    """
    def __init__(self,left,right,data,labels,col_names, label_names, \
                 label_support, ovr_class):
        self.left = left
    ...
    def compute_confidence(self,data,labels):
        ...
        for i,row in enumerate(data):
            ...
            if all_ok:
                self.ante_support += 1
                if self.right == 0:
                    if not labels[i] == self.ovr_class:
                        joint_support += 1
                else:
    ...
    def compute_lift(self,data,labels):
        print(self.right-1)
        if self.label_support[self.right-1] > 0:
